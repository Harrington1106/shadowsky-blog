...
// Video Data
// Check if data is injected via PHP
const defaultVideos = (window.videoLoaderOptions && window.videoLoaderOptions.videos) ? window.videoLoaderOptions.videos : [
    {
        id: 1,
...
// Favorite Video Data
const favoriteVideos = (window.videoLoaderOptions && window.videoLoaderOptions.favorites) ? window.videoLoaderOptions.favorites : [
    {
        id: 101,
        title: "【🐇🐱】村井優X山下瞳月《地球最可爱》",
...
class VideoLoader {
    ...
    async init() {
        ...
        // Check if container is already populated (PHP SSR)
        const isSSR = this.container && this.container.children.length > 0 && this.container.querySelector('.video-item');
